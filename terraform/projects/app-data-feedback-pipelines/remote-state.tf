...
variable "remote_state_app_knowledge_graph_key_stack" {
  type        = "string"
  description = "Override app_knowledge_graph remote state path"
  default     = ""
}

variable "remote_state_app_related_links_key_stack" {
  type        = "string"
  description = "Override stackname path to app_related_links remote state"
  default     = ""
}
...
data "terraform_remote_state" "app_related_links" {
  ...
  config {
    bucket = "${var.remote_state_bucket}"
    key    = "${coalesce(var.remote_state_app_related_links_key_stack, var.stackname)}/app-related-links.tfstate"
    region = "${var.aws_region}"
  }
}
