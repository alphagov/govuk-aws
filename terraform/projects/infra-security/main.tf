...
resource "aws_kms_key" "authenticating_proxy_documentdb" {
  description = "Encryption key for Authenticating Proxy DocumentDB"
  policy      = data.aws_iam_policy_document.kms_sops_policy.json
}
...
output "authenticating_proxy_documentdb_kms_key_arn" {
  value       = aws_kms_key.authenticating_proxy_documentdb.arn
  description = "The ARN of the Authenticating Proxy DocumentDB KMS key"
}
