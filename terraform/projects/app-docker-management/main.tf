...
resource "aws_elb" "docker_management_etcd_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_docker_management_etcd_elb_id}"]
  internal        = "true"
...
module "docker_management" {
  ...
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_docker_management_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
