...
module "db-admin" {
  ...
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_db-admin_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
...
resource "aws_iam_role_policy_attachment" "read_from_staging_content_publisher_active_storage_from_integration_iam_role_policy_attachment" {
  count      = "${var.aws_environment == "integration" ? 1 : 0}"
  role       = "${module.db-admin.instance_iam_role_name}"
  policy_arn = "${data.terraform_remote_state.infra_content_publisher_active_storage_buckets.outputs.staging_content_publisher_active_storage_bucket_reader_policy_arn}"
}
...
resource "aws_iam_role_policy_attachment" "read_write_from_integration_content_publisher_active_storage_from_integration_iam_role_policy_attachment" {
  count      = "${var.aws_environment == "integration" ? 1 : 0}"
  role       = "${module.db-admin.instance_iam_role_name}"
  policy_arn = "${data.terraform_remote_state.infra_content_publisher_active_storage_buckets.outputs.integration_content_publisher_active_storage_bucket_reader_writer_policy_arn}"
}
...
resource "aws_iam_role_policy_attachment" "read_write_from_staging_content_publisher_active_storage_from_staging_iam_role_policy_attachment" {
  count      = "${var.aws_environment == "staging" ? 1 : 0}"
  role       = "${module.db-admin.instance_iam_role_name}"
  policy_arn = "${data.terraform_remote_state.infra_content_publisher_active_storage_buckets.outputs.staging_content_publisher_active_storage_bucket_reader_writer_policy_arn}"
}
...
resource "aws_iam_role_policy_attachment" "read_from_production_content_publisher_active_storage_from_staging_iam_role_policy_attachment" {
  count      = "${var.aws_environment == "staging" ? 1 : 0}"
  role       = "${module.db-admin.instance_iam_role_name}"
  policy_arn = "${data.terraform_remote_state.infra_content_publisher_active_storage_buckets.outputs.production_content_publisher_active_storage_bucket_reader_policy_arn}"
}
