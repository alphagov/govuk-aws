...
resource "aws_elb" "email-alert-api_elb_internal" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_email-alert-api_elb_internal_id}"]
  internal        = "true"
...
module "email-alert-api" {
  ...
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_email-alert-api_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
...
resource "aws_security_group_rule" "email-alert-api-rds_ingress_email-alert-api_postgres" {
  ...
  security_group_id        = "${data.aws_security_group.email-alert-api-rds.id}"
  source_security_group_id = "${data.terraform_remote_state.infra_security_groups.outputs.sg_email-alert-api_id}"
}
