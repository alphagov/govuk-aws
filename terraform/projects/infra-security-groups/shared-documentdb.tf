...
# Allow asset manager on backend machines to access the shared documentdb cluster
resource "aws_security_group_rule" "shared-documentdb_ingress_backend_asset_manager" {
  type      = "ingress"
  from_port = 27017
  to_port   = 27017
  protocol  = "tcp"
