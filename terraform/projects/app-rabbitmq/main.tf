...
resource "aws_elb" "rabbitmq_elb" {
  ...
  security_groups = [data.terraform_remote_state.infra_security_groups.outputs.sg_rabbitmq_elb_id]
  internal        = "true"
...
module "rabbitmq" {
  ...
  instance_security_group_ids   = [data.terraform_remote_state.infra_security_groups.outputs.sg_rabbitmq_id, data.terraform_remote_state.infra_security_groups.outputs.sg_management_id]
...
resource "aws_network_interface" "rabbitmq-1_eni" {
  ...
  subnet_id       = lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.rabbitmq_1_reserved_ips_subnet)
...
resource "aws_network_interface" "rabbitmq-2_eni" {
  count           = var.federation ? 1 : 0
  subnet_id       = lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.rabbitmq_2_reserved_ips_subnet)
...
resource "aws_network_interface" "rabbitmq-3_eni" {
  count           = var.federation ? 1 : 0
  subnet_id       = lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.rabbitmq_3_reserved_ips_subnet)
