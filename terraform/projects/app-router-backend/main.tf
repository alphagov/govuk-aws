...
resource "aws_elb" "router_api_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_router-api_elb_id}"]
  internal        = "true"
...
resource "aws_network_interface" "router-backend-1_eni" {
  subnet_id       = "${lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.router-backend_1_reserved_ips_subnet)}"
...
module "router-backend-1" {
  ...
  instance_subnet_ids           = "${matchkeys(values(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), keys(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), list(var.router-backend_1_subnet))}"
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_router-backend_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
...
resource "aws_network_interface" "router-backend-2_eni" {
  subnet_id       = "${lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.router-backend_2_reserved_ips_subnet)}"
...
module "router-backend-2" {
  ...
  instance_subnet_ids           = "${matchkeys(values(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), keys(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), list(var.router-backend_2_subnet))}"
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_router-backend_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
  instance_type                 = "${var.instance_type}"
  instance_additional_user_data = "${join("\n", null_resource.user_data.*.triggers.snippet)}"
  instance_elb_ids_length       = "1"
...
resource "aws_network_interface" "router-backend-3_eni" {
  subnet_id       = "${lookup(data.terraform_remote_state.infra_networking.outputs.private_subnet_reserved_ips_names_ids_map, var.router-backend_3_reserved_ips_subnet)}"
...
module "router-backend-3" {
  ...
  instance_subnet_ids           = "${matchkeys(values(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), keys(data.terraform_remote_state.infra_networking.outputs.private_subnet_names_ids_map), list(var.router-backend_3_subnet))}"
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_router-backend_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
  instance_type                 = "${var.instance_type}"
  instance_additional_user_data = "${join("\n", null_resource.user_data.*.triggers.snippet)}"
  instance_elb_ids_length       = "1"
