...
resource "aws_docdb_cluster_instance" "cluster_instances" {
  ...
  identifier         = "router-documentdb-${count.index}"
...
resource "aws_docdb_subnet_group" "cluster_subnet" {
  name       = "router-documentdb-${var.aws_environment}"
...
resource "aws_docdb_cluster_parameter_group" "parameter_group" {
  family      = "docdb3.6"
  name        = "router-documentdb-parameter-group"
  description = "Router DocumentDB cluster parameter group"
...
resource "aws_docdb_cluster" "cluster" {
  cluster_identifier              = "router-documentdb-${var.aws_environment}"
  ...
  kms_key_id                      = "${data.terraform_remote_state.infra_security.router_documentdb_kms_key_arn}"
  vpc_security_group_ids          = ["${data.terraform_remote_state.infra_security_groups.sg_router_documentdb_id}"]
  ...
  tags = {
    Service  = "router documentdb"
    Customer = "router"
    Name     = "router-documentdb"
    Source   = "app-router-documentdb"
  }
}

resource "aws_route53_record" "share-documentdb_internal_service_cname" {
  zone_id = "${data.aws_route53_zone.internal.zone_id}"
  name    = "router-documentdb.${var.internal_domain_name}"
  type    = "CNAME"
  ttl     = 300
