output "broker_id" {
  ...
  value       = aws_mq_broker.govuk_crawler_amazonmq.id
}

output "console_url" {
  description = "URL of the RabbitMQ web management UI"
  value       = aws_mq_broker.govuk_crawler_amazonmq.instances.0.console_url
}

output "amqp_endpoint" {
  description = "AMQP URL for connecting a client app to the broker"
  value       = aws_mq_broker.govuk_crawler_amazonmq.instances.0.endpoints.0
}
...
output "internal_domain_name" {
  ...
  value       = aws_route53_record.govuk_crawler_amazonmq_internal_root_domain_name.fqdn
}

#----------------------------------------------------------------------
# These outputs are marked sensitive, so will not be shown in STDOUT 
# by default. To retrieve them, use `terraform output -json`
output "govuk_crawler_amazonmq_passwords" {
  description = "Generated passwords for each RabbitMQ user account. Use `terraform output -json | jq '.govuk_crawler_amazonmq_passwords.value'` to retrieve the values."
  sensitive   = true
  value       = local.govuk_crawler_amazonmq_passwords
}
