...
resource "aws_security_group_rule" "puppetmaster_ingress_offsite-ssh_22" {
  ...
  to_port                  = "22"
  protocol                 = "tcp"
  source_security_group_id = "${data.terraform_remote_state.infra_security_groups.outputs.sg_offsite_ssh_id}"
  security_group_id        = "${data.terraform_remote_state.infra_security_groups.outputs.sg_puppetmaster_id}"
}
...
resource "aws_elb" "puppetmaster_internal_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_puppetmaster_elb_id}"]
  internal        = "true"
...
module "puppetmaster" {
  ...
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_puppetmaster_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
