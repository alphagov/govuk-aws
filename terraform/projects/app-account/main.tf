...
resource "aws_elb" "account_elb_internal" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_account_elb_internal_id}"]
  internal        = "true"
...
module "account" {
  ...
  instance_security_group_ids   = ["${data.terraform_remote_state.infra_security_groups.outputs.sg_account_id}", "${data.terraform_remote_state.infra_security_groups.outputs.sg_management_id}"]
...
resource "aws_security_group_rule" "account-api-rds_ingress_account_postgres" {
  ...
  security_group_id        = "${data.aws_security_group.account-api-rds.id}"
  source_security_group_id = "${data.terraform_remote_state.infra_security_groups.outputs.sg_account_id}"
}
