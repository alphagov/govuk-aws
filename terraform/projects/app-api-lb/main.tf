...
resource "aws_elb" "api-lb_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.govuk_security_groups.sg_api-lb_elb_id}"]
  internal        = "true"
...
module "api-lb" {
  ...
  instance_security_group_ids          = ["${data.terraform_remote_state.govuk_security_groups.sg_api-lb_id}", "${data.terraform_remote_state.govuk_security_groups.sg_management_id}"]
