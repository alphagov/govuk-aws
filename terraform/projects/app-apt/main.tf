...
resource "aws_elb" "apt_external_elb" {
  ...
  access_logs {
    ...
    bucket_prefix = "elb/${var.stackname}-apt-external-elb"
    interval      = 60
  }
...
resource "aws_elb" "apt_internal_elb" {
  ...
  access_logs {
    bucket        = "${data.terraform_remote_state.infra_monitoring.aws_logging_bucket_id}"
    bucket_prefix = "elb/${var.stackname}-apt-internal-elb"
    interval      = 60
  }
...
module "alarms-elb-apt-internal" {
  ...
  elb_name                       = "${aws_elb.apt_internal_elb.name}"
...
module "alarms-elb-apt-external" {
  ...
  alarm_actions                  = ["${data.terraform_remote_state.infra_monitoring.sns_topic_cloudwatch_alarms_arn}"]
  elb_name                       = "${aws_elb.apt_external_elb.name}"
  httpcode_backend_4xx_threshold = "0"
  httpcode_backend_5xx_threshold = "50"
  httpcode_elb_4xx_threshold     = "0"
  httpcode_elb_5xx_threshold     = "50"
