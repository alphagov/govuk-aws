...
resource "aws_elb" "publishing-api_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.infra_security_groups.sg_publishing-api_elb_id}"]
  internal        = "true"
...
resource "aws_route53_record" "service_record" {
  ...
  alias {
    name                   = "${aws_elb.publishing-api_elb.dns_name}"
    zone_id                = "${aws_elb.publishing-api_elb.zone_id}"
...
output "publishing-api_elb_address" {
  value       = "${aws_elb.publishing-api_elb.dns_name}"
