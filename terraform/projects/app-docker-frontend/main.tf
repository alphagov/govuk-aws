...
resource "aws_elb" "docker-frontend_elb" {
  name            = "${var.stackname}-docker-frontend"
  subnets         = ["${data.terraform_remote_state.infra_networking.private_subnet_ids}"]
  security_groups = ["${data.terraform_remote_state.infra_security_groups.sg_docker-frontend_elb_id}"]
  internal        = "true"
  ...
  tags = "${map("Name", "${var.stackname}-docker-frontend", "Project", var.stackname, "aws_environment", var.aws_environment, "aws_migration", "docker_frontend")}"
}
...
resource "aws_route53_record" "service_record" {
  ...
  alias {
    name                   = "${aws_elb.docker-frontend_elb.dns_name}"
    zone_id                = "${aws_elb.docker-frontend_elb.zone_id}"
...
module "docker-worker-frontend" {
  ...
  instance_key_name             = "${var.stackname}-docker-frontend"
  instance_public_key           = "${var.ssh_public_key}"
  instance_additional_user_data = "${join("\n", null_resource.user_data.*.triggers.snippet)}"
  instance_elb_ids              = ["${aws_elb.docker-frontend_elb.id}"]
...
output "docker-frontend_elb_dns_name" {
  value       = "${aws_elb.docker-frontend_elb.dns_name}"
