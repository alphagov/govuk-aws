...
resource "aws_elb" "backend-lb_elb" {
  ...
  security_groups = ["${data.terraform_remote_state.govuk_security_groups.sg_backend-lb_elb_id}"]
  internal        = "true"
  ...
  tags = "${map("Name", "${var.stackname}-backend-lb", "Project", var.stackname, "aws_migration", "backend_lb")}"
}
...
module "backend-lb" {
  ...
  default_tags                         = "${map("Project", var.stackname, "aws_stackname", var.stackname, "aws_migration", "backend_lb", "aws_hostname", "backend-lb-1")}"
  instance_subnet_ids                  = "${data.terraform_remote_state.govuk_networking.private_subnet_ids}"
  instance_security_group_ids          = ["${data.terraform_remote_state.govuk_security_groups.sg_backend-lb_id}", "${data.terraform_remote_state.govuk_security_groups.sg_management_id}"]
  ...
  instance_elb_ids                     = ["${aws_elb.backend-lb_elb.id}"]
  asg_max_size                         = "2"
  asg_min_size                         = "2"
