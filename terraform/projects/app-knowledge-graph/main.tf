...
data "aws_iam_policy_document" "knowledge-graph_register_instance_with_elb_policy_document" {
  statement {
    ...
    resources = [
      ...
      "${aws_elb.knowledge-graph-lab_external.arn}",
    ]
  }
}
...
resource "aws_route53_record" "knowledge_graph_lab_service_record_external" {
  zone_id = "${data.aws_route53_zone.external.zone_id}"
  name    = "knowledge-graph-lab.${var.external_domain_name}"
  type    = "A"
...
resource "aws_elb" "knowledge-graph-lab_external" {
  name                = "${var.stackname}-knowledge-graph-lab-ext"
  internal            = false
  connection_draining = true
